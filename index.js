...
translate(args, {to: 'zh-cn'}).then(res => {
    let embed = new Discord.MessageEmbed()
    .setTitle("Translated")
    .addField("Text",args,false)
    .addField("Translation",res,false)
    .setColor(`RANDOM`)
}).catch(err => {
...
message.channel.send(embed)
 ...
 return message.channel.send("No results for the query")
  ...
  let qu = args.join(" ").split(" ")
  const op = message.content.replace("s?wiki","").replace(/,/gi," ")
  const queries = qu.join("_")
  let doc = await wtf.fetch(op, 'en')
  if (doc == null) {
    return message.channel.send("Couldnt find wiki for given query")
  }
let results = doc.text()
let res = results
let sem = res.split('\n');
let final = sem.shift()
let embed = new Discord.MessageEmbed()
embed.setAuthor(`Wiki for ${args.join(" ")}`)
embed.setTitle(args.join(" ").toUpperCase())
embed.setDescription(final)
embed.setFooter(`Requested by ${message.author.username}`,message.author.displayAvatarURL())
embed.setTimestamp()
embed.setColor(`RANDOM`)
embed.setURL("https://en.m.wikipedia.org/wiki/" + args.join(" "))
if (args == "") {
  return message.channel.send("Give me a query to search on wikipedia")
} else if (doc == null) {
return message.channel.send("Couldnt find a wiki on requested search query")
...
return text.split(thing).join(stuff)
  ...
  const  googleIt = require('google-it')
googleIt({'query': args.join(" ") + "definition"}).then(results => {;
let res = results[0]; 
let title = res.title;
let description = res.snippet;
let url = res.link;
...
embed.setFooter(`Requested by ${message.author.username}`,message.author.displayAvatarURL())
embed.setTimestamp()
embed.setAuthor("Definition for " + args.join(" "));
message.channel.send(embed)
}).catch(e => {
 return message.channel.send("No definitions for the query")
})
  ...
  let callingcode = list.callingCodes.shift();
  ...
  embed.setAuthor(`${args} the country ${status}`)
  ...
  const [ hello ] = await fetch('https://restcountries.eu/rest/v2/name/dagtdaftfatfatdgayftaftaudvaugvfagfdfatvtavgadgtfdtafdtyaftfatvagdvagvftaftadagfafvcgavdagvfatvfgavgfavdgav')
    .catch(err => message.channel.send("fuck off"))
    .then(response => response.json()).catch(err => message.channel.send("fuck"))
